...
@router.post("/create")
async def create_inventory_application(token: Annotated[str, Header()], request: schemas.RequestInventoryApplication) -> schemas.ResponseInventoryApplication:
...
async def get_inventory_application_by_user(user_token: str) -> list[schemas.ResponseGetInventoryApplication]:
    ...
    inventory_applications = await InventoryApplication.find(InventoryApplication.user_id == ObjectId(user.id)).to_list()
